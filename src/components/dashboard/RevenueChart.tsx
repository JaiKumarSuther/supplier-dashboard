...
const RevenueChart = ({ supplierId }: { supplierId: string }) => {
  ...
  useEffect(() => {
    const fetchRevenueData = async () => {
      try {
        const res = await fetch(`http://localhost:9000/api/v1/suppliers/${supplierId}/dashboard/revenue-chart`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        const data = await res.json();
        const revenueData = data?.monthlyRevenue || Array(12).fill(0);
